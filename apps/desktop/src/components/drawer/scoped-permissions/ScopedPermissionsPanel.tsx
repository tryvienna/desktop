/**
 * ScopedPermissionsPanel — Inline permissions UI for embedding in settings drawers.
 *
 * @ai-context
 * - Renders the same scoped permissions UI as ScopedPermissionsDrawer but without DrawerContainer
 * - Wraps content in DrawerContainer with a back button to return to parent view
 * - Used by WorkstreamSettingsDrawer and GroupSettingsDrawer via internal state toggle
 */
...
import { ScopedEntityActionsGroup } from './ScopedEntityActionsGroup';
...
export function ScopedPermissionsPanel({
  scopeType,
  scopeId,
  onBack,
}: ScopedPermissionsPanelProps) {
  const {
    loading,
    getToolState,
    setPermission,
  ...
  if (loading) {
    return (
      <DrawerContainer title="Permissions" showBackButton onBack={onBack}>
        <DrawerBody>
