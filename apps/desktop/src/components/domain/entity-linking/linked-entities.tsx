 ...
 *
 * Domain-specific, extracted from @tryvienna/ui.
 */
...
import { ContentSection, Badge, Button } from '@tryvienna/ui';
...
const GET_WORKSTREAM_LINKED_ENTITIES_GQL = gql`
...
const GET_GROUP_LINKED_ENTITIES_GQL = gql`
...
export function LinkedEntities({ targetId, scope }: LinkedEntitiesProps) {
  ...
  const isWorkstream = scope === 'workstream';
  const { data, loading, refetch } = useQuery(
    isWorkstream ? GET_WORKSTREAM_LINKED_ENTITIES_GQL : GET_GROUP_LINKED_ENTITIES_GQL,
    {
      variables: isWorkstream ? { workstreamId: targetId } : { groupId: targetId },
      fetchPolicy: 'cache-and-network',
    },
  );

  const linkedEntities: LinkedEntityType[] = (
    isWorkstream
      ? (data?.workstreamLinkedEntities ?? [])
      : (data?.groupLinkedEntities ?? [])
  ).map((e: LinkedEntityRow) => toLinkedEntity(e));
  ...
  const handleLink = useCallback(
    ...
    [adapter, targetId, scope, refetch],
  );
